...
| `ImageFileURL` | string | — | External image `file:///` URL. Used with ControlType 101 **instead of** `ImageFileName`. See `../images/external-images.md` |
...
**External images**: ControlType 101 elements use `ImageFileURL` with `file:///` paths to load images from the local filesystem. Combined with `ImageRefreshDuration`, this creates live-updating displays for cameras, album art, timetables, etc. See `../images/external-images.md` for full details and examples.
